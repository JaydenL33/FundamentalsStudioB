...
def groupbyCountry(df):
    ...
    # note, commit fuckery means I'm using the encoding, countriesAndTerritories_encoding rn to make this work for demo

	# save the schema for re adding later..TODO: figure out non destructive method
	schema_str = df.schema[0]

	try:
		df = df.groupby(df.countriesAndTerritories_encoding, axis = 0).sum()
		# df = df.reset_index()
...
print(df.columns)
print(df.head())
print(df.countriesAndTerritories_encoding)
print(df.cases)
input()
