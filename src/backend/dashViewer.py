...
def makeAllThePlots():
    ...
    :author: Albert Ferguson
    :return: none. Generates image file outputs to chosen base directory in .env config.
	"""
	...
	############################################################################
		...
		if not DEBUG:
			plt.savefig(os.path.join(baseDir, "plots_pngs", "distplots WHO Indicators RAW SCALED.png"))
	...
	############################################################################
		...
		if not DEBUG:
			plt.savefig(os.path.join(baseDir, "plots_pngs", "Boxplots WHO RAW.png"))
